        ...
        if "properties" in kids:
            ...
            if isinstance(spec, list):
                if "properties" in kids["items"]:
                    spec.append(
                        handle_properties(kids["items"]["properties"], [])
                    )
                else:
                ...
                for key, value in kids.items():
                    ...
                    if key == "type":
                        if value == "string":
                            ...
                            continue
                        if len(spec) > 0 and isinstance(
                            next(iter(spec.values())), list
                        ):
...
parser = argparse.ArgumentParser(
    description="Generate CustomResource with all options from CRD"
)
parser.add_argument(
    "--crd", dest="crd", required=True, help="Path to CRD file"
)
...
cr = {
    "apiVersion": crd["spec"]["group"]
    + "/"
    + crd["spec"]["versions"][0]["name"],
    "kind": crd["spec"]["names"]["kind"],
    "metadata": {"name": "example"},
    "spec": handle_properties(
